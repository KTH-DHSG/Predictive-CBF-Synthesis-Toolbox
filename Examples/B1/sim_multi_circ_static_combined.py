"""
    ...
    Simulation of the less agile and more agile system in a multi-circular environment with static obstacles.
...
movie_name = "b1_combined_multi-circ-static_movie"
...
def multi_circ_cbf_2p8(t,x):
    ...
    cbf_values_at_x = [cbf_interpolator_2p8(x_transformed_values[i]) - circ_radii[i] for i in range(len(x_transformed_values))]
...
def multi_circ_cbf_1p12(t,x):
    ...
    # compute the CBF values for each of the obstacle circles
    cbf_values_at_x = [cbf_interpolator_1p12(x_transformed_values[i]) - circ_radii[i] for i in range(len(x_transformed_values))]
...
def multi_circ_h(t,x):
    ...
    # compute the CBF values for each of the obstacle circles
    h_values_at_x = [bicycle_cbf_module_2p8.h(x_transformed_values[i]) - circ_radii[i] for i in range(len(x_transformed_values))]
...
controller_settings_2p8['dt'] = 0.1
...
controller_settings_1p12['dt'] = 0.1
...
# Plot obstacle
x_obstacle_grid = np.linspace(controller_settings_2p8['cbf_grid_points'][0][0], controller_settings_2p8['cbf_grid_points'][0][-1], 100)
y_obstacle_grid = np.linspace(controller_settings_2p8['cbf_grid_points'][1][0], controller_settings_2p8['cbf_grid_points'][1][-1], 100)

H_values = np.array([[multi_circ_h(0.0,[xi, yi,0]) for xi in x_obstacle_grid] for yi in y_obstacle_grid])
...
plt.axhline(y=y_d, color='r', linestyle='-', label='Desired trajectory', linewidth=1)

# Plot trajectory
Bicycle.plot_trajectory_with_bicycle_markers(plt, my_bicycle_2p8, plotting_steps, marker_spacing=marker_spacing, line_color='blue', linewidth=1, marker_size=2)
Bicycle.plot_trajectory_with_bicycle_markers(plt, my_bicycle_1p12, plotting_steps, marker_spacing=marker_spacing, line_color='green', linewidth=1, marker_size=2)
...
cbf_values_along_trajectory_2p8 = np.array([multi_circ_cbf_2p8(my_bicycle_2p8.t_sol,my_bicycle_2p8.x_sol[i]) for i in range(len(my_bicycle_2p8.x_sol))])
cbf_values_along_trajectory_1p12 = np.array([multi_circ_cbf_1p12(my_bicycle_1p12.t_sol,my_bicycle_1p12.x_sol[i]) for i in range(len(my_bicycle_1p12.x_sol))])
