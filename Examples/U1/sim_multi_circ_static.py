...
movie_name = "u1_2_1p12_multi-circ-static_movie"
...
def multi_circ_cbf(t,x):
    ...
    cbf_values_at_x = [cbf_interpolator(x_transformed_values[i]) - circ_radii[i] for i in range(len(x_transformed_values))]
...
def multi_circ_h(t,x):
    ...
    # compute the CBF values for each of the obstacle circles
    h_values_at_x = [unicycle_cbf_module.h(x_transformed_values[i]) - circ_radii[i] for i in range(len(x_transformed_values))]
