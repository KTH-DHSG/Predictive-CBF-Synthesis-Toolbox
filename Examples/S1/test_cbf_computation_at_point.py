...
# set domain bounds
domain_lower_bound = np.array([-20,-20])
domain_upper_bound = np.array([20,20])
...
sample_point = np.array([-2,0])

tic = time.time()
cbf_value, u_opt = CBFcomputation.computeCbfAtPoint(opti_object, sample_point, warmStartInputTrajectories)
toc = time.time()
_, x_opt = mySingleIntegrator.simulateOverHorizon(sample_point, u_opt, myCBFmodule.dt)
...
for i in range(warmStartInputTrajectories.shape[0]):
    _, x_warmstart = mySingleIntegrator.simulateOverHorizon(sample_point, warmStartInputTrajectories[i], myCBFmodule.dt)
