"""
    ...
    Simulation of single integrator 1 and 2 in a multi-circular environment with static obstacles.
...
def multi_circ_cbf_1(t,x):
    ...
    cbf_values_at_x = [cbf_interpolator_1(x_transformed_values[i]) - circ_radii[i] for i in range(len(x_transformed_values))]
...
def multi_circ_cbf_2(t,x):
    ...
    # compute the CBF values for each of the obstacle circles
    cbf_values_at_x = [cbf_interpolator_2(x_transformed_values[i]) - circ_radii[i] for i in range(len(x_transformed_values))]
...
def multi_circ_h(t,x):
    ...
    # compute the CBF values for each of the obstacle circles
    h_values_at_x = [sintegrator_cbf_module_1.h(x_transformed_values[i]) - circ_radii[i] for i in range(len(x_transformed_values))]

    return np.nanmin(h_values_at_x)

# Controller settings example 1
...
controller_settings_1['dt'] = 0.5
controller_settings_1['step_size'] = 0.5

# Controller settings example 2
...
H_values = np.array([[multi_circ_h(0.0,[xi, yi]) for xi in x_obstacle_grid] for yi in y_obstacle_grid])
...
cbf_values_along_trajectory_1 = np.array([multi_circ_cbf_1(my_sintegrator_1.t_sol,my_sintegrator_1.x_sol[i]) for i in range(len(my_sintegrator_1.x_sol))])
cbf_values_along_trajectory_2 = np.array([multi_circ_cbf_2(my_sintegrator_2.t_sol,my_sintegrator_2.x_sol[i]) for i in range(len(my_sintegrator_2.x_sol))])
...
ax[0].grid(True)
ax[0].set_ylabel(r'$u_{baseline} - u_{safe}$')
ax[0].set_xlim([0,25])
...
plt.plot(t_filtered_1, distance_to_obstacle_along_trajectory_filtered_1, label=r"Single integrator with $v_{x} \in [-2,2]$", color='blue')
plt.plot(t_filtered_2, distance_to_obstacle_along_trajectory_filtered_2, label=r"Single integrator with $v_{x} \in [1,2]$", color='green')
plt.xlim([0,25])
plt.xlabel('Time [s]')
plt.legend()
plt.show(block=False)
...
for i in tqdm(range(0,len(my_sintegrator_1.t_sol),data_steps), desc="Creating frames", unit="frame"):
    ...
    SingleIntegrator.plot_trajectory_at_indices_withmarkers(plt, my_sintegrator_1, [i], color='blue', marker_size=4)
    SingleIntegrator.plot_trajectory_at_indices_withmarkers(plt, my_sintegrator_2, [i], color='green', marker_size=4)
