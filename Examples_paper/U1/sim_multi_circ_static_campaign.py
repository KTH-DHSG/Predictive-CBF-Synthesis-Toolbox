"""
    ...
    Code for simulating unicycles avoiding multiple static circular obstacles.
...
cbf_module_filename = "2025-11-11_06-30-41_u1_cbfm_1p12.json"

cbf_module_folder_path = r'Examples_paper\U1\Data'

movie_name = "u1_2_1p12_multi-circ-static-cpg_movie"
...
circ_radii = [r2(t_constraint), r2(t_constraint),
            r1(t_constraint), r1(t_constraint), r1(t_constraint),
            r3(t_constraint), r3(t_constraint),
            r2(t_constraint), r3(t_constraint), r2(t_constraint),
            r1(t_constraint), r1(t_constraint),
            r3(t_constraint), r2(t_constraint), r3(t_constraint),
            r3(t_constraint), r3(t_constraint), 
            r1(t_constraint), r1(t_constraint), r1(t_constraint),
            r2(t_constraint), r2(t_constraint)]
...
def multi_circ_cbf(t,x):
    ...
    circ_radii_local = [circ_radii[i] for i in range(len(circ_centers)) if np.linalg.norm(x[0:2]-circ_centers[i][0:2]) <= dist_max]
...
def multi_circ_h(t,x):
    ...
    # compute the CBF values for each of the obstacle circles
    h_values_at_x = [unicycle_cbf_module.h(x_transformed_values[i]) - circ_radii_local[i] for i in range(len(x_transformed_values))]
...
controller_settings['alpha'] = lambda b: alpha(b, c=1.0, gamma=unicycle_cbf_module.gamma)
...
u_baseline_sol_array = [np.zeros((0,my_unicycle.u_dim)) for _ in range(len(x0_array))]

my_unicycle_array = [copy.deepcopy(my_unicycle) for _ in range(len(x0_array))]

# Set initial time and state in dynamic systen
for i in range(len(x0_array)):
    my_unicycle_array[i].t0 = t0
    my_unicycle_array[i].x0 = x0_array[i]
    my_unicycle_array[i].reset()
...
for step in tqdm(range(n_steps), desc="Simulating", unit="step"):
    ...
    u_baseline_array = [Unicycle.u_follow_straight_line(x=current_state_array[i], v_d=v_d, y_d=y_d[i], rot=x0_array[i][2]) for i in range(len(x0_array))]
    ...
    for i in range(len(x0_array)):
        current_state_array[i] = my_unicycle_array[i].simulate(x=current_state_array[i], u=u_safe_array[i], dt=dt, saveSolution=True)
...
for i, center in enumerate(circ_centers):
    radius = circ_radii[i]
    circ = Circle((center[0], center[1]), radius, fill=True, facecolor='gray', edgecolor='black', linewidth=1, linestyle='-', alpha=0.9)
    ax.add_patch(circ)
...
for i in range(len(x0_array)):
    ax.axline(x0_array[i][0:2], [0.0, y_d[i]], color=color_cycle_light[i], linestyle='--', linewidth=1)

# Plot trajectory
for i in range(len(x0_array)):
    my_unicycle = my_unicycle_array[i]
    Unicycle.plot_trajectory_as_triangular_marker(plt, my_unicycle, marker_spacing=marker_spacing, line_color=color_cycle[i], linewidth=2, marker_shape=(1.5,0.8))
...
cbf_values_along_trajectory_array = [np.array([multi_circ_cbf(my_unicycle.t_sol[i],my_unicycle.x_sol[i]) for i in range(len(my_unicycle.x_sol))]) for my_unicycle in my_unicycle_array]

distance_to_obstacle_along_trajectory_array = [np.array([multi_circ_h(my_unicycle.t_sol[i], my_unicycle.x_sol[i,:]) for i in range(len(my_unicycle.x_sol))]) for my_unicycle in my_unicycle_array]
...
t_filtered_array = [my_unicycle.t_sol[valid_indices] for my_unicycle, valid_indices in zip(my_unicycle_array, valid_indices_array)]
...
for i in range(len(x0_array)):
    ...
    ax.plot(my_unicycle.t_sol, cbf_values_along_trajectory, label='CBF value')
    color = ax.lines[-1].get_color()
    ax.plot(t_filtered, distance_to_obstacle_along_trajectory_filtered, label='Distance to obstacle', linestyle='--', color=color)
ax.grid(True)
ax.set_xlim([0, 40])
ax.set_ylim([-1,4.0])
...
frames_dir = "frames_u1_static_cpg"
os.makedirs(frames_dir, exist_ok=True)

H_values_static = np.array([[multi_circ_h(my_unicycle.t_sol[0],[xi, yi,0]) for xi in x_obstacle_grid] for yi in y_obstacle_grid])
...
for i in tqdm(range(0,len(my_unicycle.t_sol),data_steps), desc="Creating frames", unit="frame"):
    ...
    ax.contourf(x_obstacle_grid, y_obstacle_grid, H_values_static, levels=[-np.inf,0], colors='gray', alpha=0.5)  # plot the obstacle
    ax.contour(x_obstacle_grid, y_obstacle_grid, H_values_static, levels=[0], colors='k') 
    ...
    for k in range(len(x0_array)):
        my_unicycle = my_unicycle_array[k]
        ax.plot(my_unicycle.x_sol[:i,0], my_unicycle.x_sol[:i,1], color=color_cycle_light[k], linewidth=2)  # Draw trajectory until current time step
        Unicycle.plot_trajectory_at_indices_as_triangular_marker(plt, my_unicycle, [i], marker_shape=(1.5,0.8))
