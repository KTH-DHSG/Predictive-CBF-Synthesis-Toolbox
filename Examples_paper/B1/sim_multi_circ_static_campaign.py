"""
    ...
    Simulation example with static circular obstacles.
...
movie_name = "b1_multi-circ-static-cpg_movie"
...
for i in range(len(x0_array)):
    ax.axline(x0_array[i][0:2], [0.0, y_d[i]], color=color_cycle_dark[i], linestyle='--', linewidth=1)
...
frames_dir = "frames_b1_static_cpg"
os.makedirs(frames_dir, exist_ok=True)

H_values_static = np.array([[multi_circ_h(my_bicycle.t_sol[0],[xi, yi,0]) for xi in x_obstacle_grid] for yi in y_obstacle_grid])
...
for i in tqdm(range(0,len(my_bicycle.t_sol),data_steps), desc="Creating frames", unit="frame"):
    ...
    for k in range(len(x0_array)):
        ...
        ax.plot(my_bicycle.x_sol[:i,0], my_bicycle.x_sol[:i,1], color=color_cycle[k], linewidth=2)  # Draw trajectory until current time step
